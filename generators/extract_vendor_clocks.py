...
class VendorClockExtractor:
    ...
    def extract_multiline_macro(self, lines, start_idx, macro_name):
        """Extract a multi-line macro definition ending with ');'."""
        block = ""
        i = start_idx
        ...
        # Flatten to single line for regex matching
    ...
    def extract_sunxi_ccu_m(self, flat_line):
    ...
    def extract_clk_fixed_factor(self, flat_line):
    ...
    def extract_gate(self, flat_line):
        """Extract a gate clock definition from flattened macro."""
        match = re.match(
            r'static SUNXI_CCU_GATE\(\w+,\s*"([^"]+)",\s*"([^"]+)",\s*(0x[0-9a-fA-F]+),\s*(?:BIT\()?([\d]+)(?:\))?,\s*(\d+)\)',
            flat_line,
        )
        if match:
            return {
    ...
    def parse_file(self, filepath):
        ...
        while i < len(lines):
            ...
            # Extract PLL (ccu_nm) - multi-line struct
            ...
            # Extract single-line macros
            if "SUNXI_CCU_M(" in line and "struct ccu_nm" not in line:
                clk = self.extract_sunxi_ccu_m(" ".join(line.split()))
                if clk:
                    self.clocks.append(clk)
                i += 1
                continue
            ...
            if "CLK_FIXED_FACTOR(" in line:
                clk = self.extract_clk_fixed_factor(" ".join(line.split()))
                if clk:
                    self.clocks.append(clk)
                i += 1
                continue

            # Extract multi-line gate macros
            if "SUNXI_CCU_GATE(" in line:
                flat, i = self.extract_multiline_macro(lines, i, "SUNXI_CCU_GATE")
                clk = self.extract_gate(flat)
                if clk:
                    self.clocks.append(clk)
                else:
                    # Try simpler single-line extraction
                    clk = self.extract_gate(" ".join(line.split()))
                    if clk:
                        self.clocks.append(clk)
                continue
