...
ROOT_COMPAT = "allwinner,sun60i-a733-ccu"
...
"""
    ...
    def emit_gate_with_key(self, c: Dict) -> str:
        ...
        key_literal = KEY_VALUE_LITERALS.get(key_token, key_token)
...
"""
    ...
    def emit_ccu_clks(self) -> str:
        ...
        body = "\n".join(lines)
        return f"static struct ccu_common *sun60i_a733_ccu_clks[] = {{\n{body}\n}};\n\n"
    ...
    def emit_hw_clks(self) -> str:
        ...
        body = "\n".join(lines)
        return f"""static struct clk_hw_onecell_data sun60i_a733_hw_clks = {{
	.num	= CLK_NUMBER,
	.hws	= {{
{body}
	}},
...
"""
    ...
    def emit_desc(self) -> str:
        resets_ptr = "sun60i_a733_ccu_resets" if self.resets else "NULL"
        resets_num = "ARRAY_SIZE(sun60i_a733_ccu_resets)" if self.resets else "0"
        return f"""static const struct sunxi_ccu_desc sun60i_a733_ccu_desc = {{
...
"""
    ...
    def emit_probe(self) -> str:
        return f"""static int sun60i_a733_ccu_probe(struct platform_device *pdev)
{{
	void __iomem *reg;
	int ret;
...
def main() -> int:
    ...
    extracted = load_json(extracted_path) if extracted_path.exists() else {"clocks": []}
