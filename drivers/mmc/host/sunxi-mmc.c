...
#define SDXC_REG_CSDC		0x054 /* CRC status detect control (SMHC v5p3) */
#define SDXC_REG_EDSD		0x10C /* HS400 / eMMC delay select (SMHC v5p3) */
...
#define SDXC_HS400_MD_EN		BIT(31)
#define SDXC_CRC_DET_PARA_MASK		GENMASK(3, 0)
#define SDXC_CRC_DET_PARA_OTHER		0x3
...
static void sunxi_mmc_dump_errinfo(struct sunxi_mmc_host *host)
{
	...
	if ((host->int_sum & SDXC_INTERRUPT_ERROR_BIT) ==
		...
		/*
		 * Temporary A733 Wi-Fi bring-up trace: mainline suppresses
		 * these RTOs, but SDIO enumeration lives exactly here.
		 */
		if (host->cfg->needs_2x_clk)
			dev_info(mmc_dev(host->mmc),
				 "a733-sdio: cmd=%u arg=0x%x int=%08x status=%08x clock=%u actual=%u mclk=%lu drv_dl=%08x ntsr=%08x samp_dl=%08x\n",
				 cmd->opcode, cmd->arg, host->int_sum,
				  ...
				  struct mmc_ios *ios)
{
	...
	/*
	 * A733 SMHC v5p3 (vendor sunxi-mmc-v5p3x): program drive delay and
	 * NTSR sample phases. Vendor writes REG_DRV_DL with the module clock
	 * gated (sunxi_r_op); leaving the clock running can leave the delay
	 * line inert even when the register readback looks correct.
	 */
	if (host->cfg->needs_2x_clk) {
		clk_disable_unprepare(host->clk_mmc);

		rval = mmc_readl(host, REG_DRV_DL);
		rval |= SDXC_CMD_DRV_PH_SEL;	/* CMD 180° */
		rval &= ~SDXC_DAT_DRV_PH_SEL;	/* DAT 90° */
		mmc_writel(host, REG_DRV_DL, rval);

		ret = clk_prepare_enable(host->clk_mmc);
		if (ret) {
			dev_err(mmc_dev(mmc), "re-enable mmc clk err %d\n", ret);
			return ret;
		}
		...
		/*
		 * Vendor sunxi-mmc-v5p3x programs these on every set_rate for
		 * non-HS400 (SDIO identify included): clear HS400 mode and set
		 * CRC detect parameter to OTHER (3). Mainline previously left
		 * CSDC/EDSD at reset, which diverged from the working path.
		 */
		rval = readl(host->reg_base + SDXC_REG_EDSD);
		rval &= ~SDXC_HS400_MD_EN;
		writel(rval, host->reg_base + SDXC_REG_EDSD);

		rval = readl(host->reg_base + SDXC_REG_CSDC);
		rval &= ~SDXC_CRC_DET_PARA_MASK;
		rval |= SDXC_CRC_DET_PARA_OTHER;
		writel(rval, host->reg_base + SDXC_REG_CSDC);
	}
	...
	/*
	 * Vendor v5p3 leaves SAMP_DL alone (software delay path commented
	 * out). Forcing SAMP_DL_SW_EN with delay 0 diverges from that reset
	 * state, so skip calibration on the A733 2x-clock controllers.
	 */
	if (!host->cfg->needs_2x_clk) {
...
static int sunxi_mmc_enable(struct sunxi_mmc_host *host)
{
	...
	if (!IS_ERR_OR_NULL(host->clk_store)) {
		ret = clk_prepare_enable(host->clk_store);
		if (ret) {
			dev_err(host->dev, "Enable store clk err %d\n", ret);
			goto error_disable_clk_ahb;
		}
	}

	if (!IS_ERR_OR_NULL(host->clk_mbus)) {
		ret = clk_prepare_enable(host->clk_mbus);
		if (ret) {
			dev_err(host->dev, "Enable mbus clk err %d\n", ret);
			goto error_disable_clk_store;
		}
	}

	if (!IS_ERR_OR_NULL(host->clk_msi)) {
		ret = clk_prepare_enable(host->clk_msi);
		if (ret) {
			dev_err(host->dev, "Enable msi clk err %d\n", ret);
...
error_disable_clk_mmc:
	clk_disable_unprepare(host->clk_mmc);
error_disable_clk_msi:
	if (!IS_ERR_OR_NULL(host->clk_msi))
		clk_disable_unprepare(host->clk_msi);
				      ...
				      struct platform_device *pdev)
{
	...
	/*
	 * No vmmc-supply (A733 Wi-Fi SDIO matches vendor: rails are always-on
	 * PMIC outputs, not MMC-tied). Without a regulator, ocr_avail stays 0
	 * and mmc_select_voltage() prints "no support for card's volts".
	 * Vendor sunxi-mmc falls back to the 2.8–3.4 V OCR mask in that case.
	 */
	...
	/* Optional A733 SMHC bus clocks (vendor mmc_store/mmc_mbus/mmc_msi_lite). */
	host->clk_store = devm_clk_get_optional(&pdev->dev, "mmc_store");
	if (IS_ERR(host->clk_store))
		return PTR_ERR(host->clk_store);

	host->clk_mbus = devm_clk_get_optional(&pdev->dev, "mmc_mbus");
	if (IS_ERR(host->clk_mbus))
		return PTR_ERR(host->clk_mbus);

	host->clk_msi = devm_clk_get_optional(&pdev->dev, "mmc_msi_lite");
	if (IS_ERR(host->clk_msi))
		return PTR_ERR(host->clk_msi);
			    ...
			    const char *buf, size_t count)
{
	...
	/*
	 * Match vendor sunxi_mmc_rescan_card(): only mmc_detect_change(),
	 * no host unbind/bind and no WL_REG_ON pulse.
	 */
	if (!mmc)
		return -ENODEV;
...
static int sunxi_mmc_probe(struct platform_device *pdev)
{
	...
	/*
	 * Soft rescan for soldered SDIO (Wi-Fi): vendor aicbsp calls
	 * sunxi_mmc_rescan_card() after power-on. Expose the same action
	 * without tearing the host down.
	 */
	if (host->cfg->needs_2x_clk) {
		ret = device_create_file(&pdev->dev, &dev_attr_rescan);
		if (ret)
			dev_warn(&pdev->dev, "failed to create rescan attr: %d\n",
				 ret);
	}
...
static void sunxi_mmc_remove(struct platform_device *pdev)
{
	...
	if (host->cfg->needs_2x_clk)
		device_remove_file(&pdev->dev, &dev_attr_rescan);
